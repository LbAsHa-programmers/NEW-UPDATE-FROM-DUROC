...
delete [interfaçage].[dbo].[CMl_ANL]
     WHERE  [ID] not IN ( SELECT [ID] FROM [dbx3v12].[dbo].[Ycommercial_annuel])

--COMMANDE_GROUP

delete [interfaçage].[dbo].[CMD_GRP]
     WHERE  [ID] not IN ( SELECT [ID] FROM [dbx3v12].[dbo].[YCMD_GRP])

---detail cmd
	
     delete [interfaçage].[dbo].[CMD_DTL]
     WHERE  [ID] not IN ( SELECT [ID] FROM [dbx3v12].[dbo].[YCMD_DTL])
